...
class Profile:
    ...
    def lbl_encode(self, profile_data):
        profile_data.set_index("FBID", inplace=True)
        for f in profile_data.columns:
            if f == "AGE_RANGE":
                profile_data[f] = profile_data[f].map(
                    {
                        "<= 21": 0,
                        "22": 1,
                        "23-27": 2,
    ...
    def get_profile(self, profile_link):
        ...
        profile_data = pd.read_csv(
            profile_link, header=0, converters={"FBID": str, "AGE": int, "GB": str}
        )
        profile_data = profile_data[profile_data["GB"] != "2"]
        profile_data["AGE_RANGE"] = profile_data["AGE"].map(self.age_bin)
        profile_data["RELATIONSHIP_F9"] = profile_data["RELATIONSHIP_F9"].map(
            self.rls_convert
        )
        profile_data["RELATIONSHIP_CHECK"] = (
            profile_data["RELATIONSHIP_F9"] == profile_data["RELATIONSHIP_M"]
        )
        profile_data["LOCATION_F9"].fillna("Unknown", inplace=True)
        profile_data["LOCATION_CHECK"] = (
            profile_data["LOCATION_F9"] == profile_data["LOCATION_M"]
        )
        profile_data = self.lbl_encode(profile_data)
        print(
            "Done getting profile data for training. Time taken = {:.1f}(s) \n".format(
                time.time() - start
            )
        )
        return profile_data
